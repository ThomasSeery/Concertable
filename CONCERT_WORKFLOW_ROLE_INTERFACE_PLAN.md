...
**Status:** **COMPLETE on `Refactor/ConcertWorkflowCoupling` (2026-05-02 → 2026-05-03). Ready to merge.** Role-interface refactor + artist-side VenueHire apply-checkout feature both landed. 21 originally-scoped tasks (BE1-14, FE1-7) + the `IConcertPaymentFlow` scrap (Option A) + integration tests + dev seed + UX polish (eligibility-driven button disable, apply-checkout loading state, pagination mapper). Concert tests 14/14 unit + 46/46 integration green; FE tsc clean. Branch HEAD: `3c3b4112`.

### Progress checkpoint (last updated 2026-05-03)
...
| Commit | What |
|---|---|
| `3c3b4112` | **Apply-checkout loading state**: between `confirmSetup` returning and `applyMutation` resolving, render `CheckoutAwaiting` with "Card authorised ✓ → Submitting application ⟳". No SignalR notification needed (SetupIntent is sync-confirmed, no webhook gate to wait on). |
| `1aadcacd` | **Dev seed: 4 unapplied demo opps at venue 1** (DB ids 60–63 = FlatFee/DoorSplit/Versus/VenueHire). Existing seed already covers the artist-1-applied side. Drive-by: OpportunityCard TDZ ReferenceError on `user.role` — moved `isArtistManager` below the `useAuthStore` call. |
| `260637b3` | **Apply-button eligibility wiring + apply-checkout BE guard**: renamed `GET /api/Application/can-apply/{oppId}` → `/api/Application/opportunity/{oppId}/eligibility` (matches Venue review eligibility shape — always returns `Ok<bool>`). `ApplicationService.ApplyCheckoutAsync` now runs `CanApplyAsync` before delegating, so a duplicate-apply 400s **before** Stripe SetupIntent fires (was: card authorised, then 400 — wasted Stripe call + bad UX). FE `useApply` queries the eligibility endpoint per opp; OpportunityCard renders Apply/Continue button as **disabled** instead of hidden when ineligible. |
| `c9950efb` | **Pagination mapper fix**: `OpportunityController.GetActiveByVenueId` returned raw `Pagination<OpportunityDto>` so the FE OpportunityCard tripped on undefined `opportunity.actions`. Added `IPagination<OpportunityResponse> ToResponses(IPagination<OpportunityDto>)` overload to the mapper (mirrors existing `IOpportunityMapper.ToDtosAsync` shape) and routed the list endpoint through it. |
| `dfc95be5` | **Checkout endpoint integration tests + ApplyAsync NRE fix**. 9 new tests across the 4 per-contract `Application{Type}ApiTests.cs`: AcceptCheckout 200 (timing + amount shape per contract) + ApplyCheckout 400 for FlatFee/DoorSplit/Versus; ApplyCheckout 200 (Authorize + flat HireFee) + AcceptCheckout 400 for VenueHire; full `ApplyCheckoutThenApply_ShouldStorePaymentMethodOnPrepaidApplication` flow (creates fresh VenueHire opp, runs apply-checkout, applies with PM, asserts `PrepaidApplication` exists with PM). The full-flow test surfaced a pre-existing NRE in `ApplicationService.ApplyAsync` — `ToDtoAsync` against fresh entity (no `Artist`/`Opportunity` navs loaded) → `BuildArtistSummary` NRE'd. Affected every artist-side apply path; hidden because no apply integration tests existed. Fix: re-fetch via `applicationRepository.GetByIdAsync(application.Id)` after save. |
| `9ce4a572` | **Apply-checkout feature for artist-side VenueHire + scrap `IConcertPaymentFlow`**: BE1-14 + FE1-7 + BE7b. Renamed `AcceptCheckout` DTO → `Checkout`. `PaymentTiming` gains `Authorize`. `IApplyCheckout`/`IAcceptCheckout` phase markers extending `ICheckout` — VenueHire declares `IApplyCheckout`, others `IAcceptCheckout`. New `ICheckoutDispatcher` (`ApplyCheckoutAsync(oppId)` + `AcceptCheckoutAsync(appId)`) replaces the nullable `IAcceptanceDispatcher.CheckoutAsync`. New `POST /api/Application/opportunity/{oppId}/checkout` endpoint. VenueHire workflow implements `CheckoutAsync` (SetupIntent for the artist). `ApplicationResponse.actions.checkout` gate tightened to `IAcceptCheckout`; `OpportunityResponse` re-introduced with `actions.checkout?` gated on `IApplyCheckout`. **`IConcertPaymentFlow` scrapped (Option A)**: keyed-DI On/Off split was hiding a coincidence (on/off ↔ payment/setup) that broke when VenueHire needed on-session + SetupIntent. Strategies + Upfront/Deferred services inject `IManagerPaymentModule` directly with explicit `PaymentSession` literals; off-session `HasStripeCustomer` precondition moved into `ManagerPaymentModule.PayAsync`. `CreateSavedCardSessionAsync` renamed `CreateSetupSessionAsync`. FE: `Opportunity.actions.checkout` drives Apply button branch (Continue → `ApplyCheckoutPage` / Apply → `applyToOpportunity`). New artist apply-checkout page renders Stripe Elements in Setup mode, posts the resulting PM back through `applyToOpportunity(oppId, pm)`. |
| `d2c5ffa7` | **Reverted the Apply URL move (`fa629ee1`)**: Apply CREATES an Application row — the opportunity is just a foreign reference, not the resource being modified. Controller owns the entity being written, not the URL parent. Apply is back at `POST /api/Application/{oppId}` on `ApplicationController`. Accept stays on ApplicationController for the same reason (transitions an Application). `OpportunityResponse` + `OpportunityActions` + `IOpportunityResponseMapper` deleted; `OpportunityController` returns `OpportunityDto` verbatim again (then re-introduced in `9ce4a572` for the apply-checkout HATEOAS link). FE: `applyToOpportunity` back in `applicationApi.ts`. **Surviving from `fa629ee1`:** `ApplyRequest`/`AcceptRequest` tightened (`PaymentMethodId` non-nullable on type, request itself nullable in controller, branch on `request is not null`); `acceptApplication` FE tightened to send body only when PM is defined; `SelectMessages()` extension. |
| `fa629ee1` | ~~Apply moved to OpportunityController~~ — **REVERTED in `d2c5ffa7`.** False consistency: URL parent ≠ controller ownership. Controller owns the entity being created/modified; "apply for an opportunity" creates an Application, not modifies an Opportunity. Reverted along with the OpportunityResponse plumbing it brought in. The non-nullable `Apply/AcceptRequest` + FE accept body fix + `SelectMessages` extension kept. |
...
| ~~13~~ | ~~Slim `IFinishOutcome` → `Task`~~ — **DONE** in `c414c77f`. `IFinishOutcome` and both records deleted; tests + dev/E2E endpoints migrated to `/e2e/payment-intent/{applicationId}`. |
| ~~14~~ | ~~DI registration audit~~ — **DONE** in `8c656fc8` pass. No leftover Executor/IFinishOutcome refs anywhere; `AddConcertModule` registers strategies under `IConcertWorkflow` keyed by `ContractType`, capability registry populated from same type dict. |
| ~~15~~ | ~~Run `./initial-migrations.ps1`~~ — **DONE** as part of #11. |
| ~~16~~ | ~~Test sweep~~ — **DONE** in `8c656fc8`. The 6 `Assert.Single(DraftCreated)` failures resolved by asserting both artist + venue notifications (renamed test methods to match). Concert integration suite 37/37 green. |
| ~~17~~ | ~~Frontend: HATEOAS-driven apply/accept~~ — **DONE** in `985d9823`. Venue accept flow now branches on `application.actions.checkout` presence (not `contract.type`). VenueHire venue-side accept now works (was broken pre-this change — navigated to checkout page that 204'd). Artist-side VenueHire apply (with PM) is **still missing entirely** in the FE — separate task, not part of #17 scope. |
...
### ~~Pending — apply-checkout feature (this branch, before merge)~~ — **ALL LANDED 2026-05-03**

All BE1-14 + FE1-7 + BE7b (`IConcertPaymentFlow` scrap) + integration tests + dev seed + UX polish landed in commits `9ce4a572` → `3c3b4112`. See "Branch + commits" table above for the per-commit breakdown. The task tables below are kept for posterity / context-on-resume; do not re-execute.

**Why this scope expanded onto this branch:** merging the role-interface refactor alone would ship a regression — the artist-side VenueHire apply path 400s today (FE POSTs `/api/Application/{oppId}` with no body → `IApplyDispatcher.ApplyAsync(oppId, artistId)` → `is ISimpleApply w` fails because VenueHire is `IPaidApply` → throws `"This contract requires a payment method at apply"`). The plumbing for `ApplyAsync(oppId, artistId, pmId)` exists, but the FE has no way to *get* a PM (no Stripe checkout endpoint at apply-time, no SetupIntent flow, no FE page, no HATEOAS hint).

**Decision (locked, do not re-litigate):** all 13 backend tasks + 7 frontend tasks land on `Refactor/ConcertWorkflowCoupling` before merge to master. No separate branch.

#### Backend tasks

| # | Task | Notes |
|---|---|---|
| BE1 | Rename DTO `AcceptCheckout` → `Checkout` | Fields unchanged (`Session`, `Payee`, `Timing`, `Amount`). Just the name is now generic enough to serve apply-side too. FE mirrors. |
| BE2 | Rename `ICheckout.CheckoutAsync(int applicationId)` → `(int contextId)` | Param rename only. Each strategy interprets contextId per its lifecycle phase: VenueHire reads it as `oppId`; FlatFee/DoorSplit/Versus read it as `appId`. |
| BE3 | Add `Authorize` to `PaymentTiming` enum | Sits alongside `Immediate`/`Deferred`. Used by VenueHire's apply-checkout (SetupIntent — authorising a card, not charging). |
| BE4 | Add phase-discriminator markers `IApplyCheckout` / `IAcceptCheckout` | **Both extend `ICheckout`** (NOT `ICheckoutable`) so they inherit the method — strategies declare exactly one phase marker and drop the bare `ICheckout` declaration. The marker carries the cast guard inside dispatchers (`is IApplyCheckout w` → `w.CheckoutAsync(oppId)`, single cast). Used by HATEOAS mappers (`registry.Has<IApplyCheckout>(ct)`) AND the dispatcher routing guard (so a guessed URL on the wrong endpoint throws). **Decisions rejected**: (a) pure markers extending `ICheckoutable` — would require a double cast in the dispatcher and let strategies declare both phases by accident; (b) use bundle markers as gate — ties HATEOAS to bundle existence, bundles aren't compulsory. |
| BE5 | Add `IApplyCheckout` to `IPrepaidConcertWorkflow` bundle | VenueHire now declares an apply-side checkout. (`IApplyCheckout : ICheckout`, so no separate `ICheckout` needed.) |
| BE6 | Add `IAcceptCheckout` to `IStandardConcertWorkflow` bundle | FlatFee/DoorSplit/Versus's existing checkout is at accept time. (Bare `ICheckout` removed from bundle in favour of the phase marker.) |
| BE7 | Add `IConcertPaymentFlow.CreateSetupSessionAsync(payerId, metadata)` | Sibling to `CreateSessionAsync`. Creates a Stripe **SetupIntent** (not PaymentIntent), returns `CheckoutSession { clientSecret, customerSession, customerId }`. Lives in `OnSessionConcertPaymentFlow` only (artist is on-session at apply-time); off-session impl throws. |
| BE8 | Implement `VenueHireConcertWorkflow.CheckoutAsync(int oppId)` | Load contract, fetch venue info via `payerLookup.GetVenueByOpportunityIdAsync(oppId)`, call `paymentFlow.CreateSetupSessionAsync(currentUser.GetId(), metadata)`, return `Checkout(session, venue, PaymentTiming.Authorize, new FlatPayment(contract.HireFee))`. |
| BE9 | Move `CheckoutAsync` from `IAcceptanceDispatcher` → new `ICheckoutDispatcher` | Two methods: `ApplyCheckoutAsync(int oppId) → Task<Checkout>` + `AcceptCheckoutAsync(int appId) → Task<Checkout>`. Both cast against the phase marker (`is IApplyCheckout w` / `is IAcceptCheckout w`) — single cast resolves the method (markers extend `ICheckout`). Throws `BadRequestException` if the contract doesn't support that phase. **Non-nullable return** — every workflow implements `ICheckout` once VenueHire is added; the marker is the routing guard. `IAcceptanceDispatcher` shrinks to just the two `AcceptAsync` overloads. |
| BE10 | Update `IApplicationService` checkout passthroughs | Drop `CheckoutAsync(int applicationId)`. Replace with `ApplyCheckoutAsync(int oppId) → Task<Checkout>` + `AcceptCheckoutAsync(int appId) → Task<Checkout>` — both pass through to the new dispatcher. Non-nullable. |
| BE11 | Add `POST /api/Application/opportunity/{oppId}/checkout` endpoint | On `ApplicationController` (the eventual PM write is against `PrepaidApplication.PaymentMethodId`, so Application owns the URL even though no Application exists yet at this point). Returns `Checkout` (200 — non-nullable; misrouted URL throws 400 via dispatcher guard). |
| BE12 | Update existing `POST /api/Application/{appId}/checkout` endpoint | Calls `applicationService.AcceptCheckoutAsync(appId)` (renamed from `CheckoutAsync`). Returns `Checkout` (no more 204 — every standard contract has accept-checkout; mistargeted contract type 400s via dispatcher guard). |
| BE13 | Re-introduce `OpportunityResponse` + `OpportunityActions(ActionLink? Checkout)` + `IOpportunityResponseMapper` | Mapper emits `actions.checkout?` link to `/api/Application/opportunity/{oppId}/checkout`, gated on `registry.Has<IApplyCheckout>(ct)` (BE4 marker). `OpportunityController` GET/POST/PUT use the mapper. **NB:** this is the OpportunityResponse plumbing that was deleted in `d2c5ffa7` — re-introducing it for the *checkout* link, not the *apply* link. The Apply URL stays at `POST /api/Application/{oppId}` (controller owns entity being created). |
| BE14 | Update `ApplicationResponseMapper.actions.checkout` registry gate | Change from generic `ICheckoutable` to specific `IAcceptCheckout` (BE4 marker). Stops emission for VenueHire (which now has `IApplyCheckout` instead). |

#### Frontend tasks

| # | Task | Notes |
|---|---|---|
| FE1 | Rename type `AcceptCheckout` → `Checkout` | Mirrors BE1. Update consumers (`ApplicationCheckoutPage`, `useCheckoutQuery`, etc). |
| FE2 | Add `Authorize` to `PaymentTiming` type | Mirrors BE3. `StripePaymentForm` gets a third branch: `timing === "Authorize"` uses `stripe.confirmSetup` (same SDK call as `Deferred` since Setup mode also doesn't charge — could collapse if cleaner). |
| FE3 | Add `applicationApi.applyCheckout(opportunityId)` | POSTs `/application/opportunity/${opportunityId}/checkout`, returns `Checkout` (non-nullable — 200 always; misrouted contract 400s). Existing `applicationApi.checkout(applicationId)` (the accept-side one) drops `\| null` for the same reason. |
| FE4 | Update `applicationApi.applyToOpportunity(oppId, paymentMethodId?)` | Add optional PM param. Send body only when defined (mirrors `acceptApplication` post-`fa629ee1`). URL unchanged: `/application/${oppId}`. |
| FE5 | Add `actions.checkout?: ActionLink` to `Opportunity` type | Mirrors BE13's response shape. |
| FE6 | Update Apply button on opportunity browse | Branch on `opportunity.actions.checkout` presence: if non-null → navigate to artist apply-checkout page; if null → submit `applyToOpportunity(oppId)` directly. Same shape as `AcceptApplicationPage`'s post-#17 branch. Stays contract-type-blind. |
| FE7 | New artist apply-checkout page | New route. Reads `opportunity.actions.checkout.href`, POSTs to it, gets `Checkout` session. Renders Stripe Elements in **Setup mode** (`stripe.confirmSetup`). On success, calls `applyToOpportunity(oppId, pmId)` → application created. Mirror 3DS handling from `useTicketCheckout` (handle `requiresAction` + `clientSecret` via `stripe.handleNextAction`). On completion, navigate to artist application detail. |

#### Test coverage — landed in `dfc95be5`

9 new tests across the 4 per-contract `Application{Type}ApiTests.cs`:
- AcceptCheckout 200 (timing + amount shape per contract) + ApplyCheckout 400 for FlatFee/DoorSplit/Versus
- ApplyCheckout 200 (Authorize + flat HireFee) + AcceptCheckout 400 for VenueHire
- Full `ApplyCheckoutThenApply_ShouldStorePaymentMethodOnPrepaidApplication` flow (creates fresh VenueHire opp, runs apply-checkout, applies with PM, asserts `PrepaidApplication` exists with PM)

`CheckoutDispatcherTests` (unit) skipped — endpoint integration tests cover the same routing-guard behaviour with less ceremony. Add later if a regression slips through.

Concert suite final: 14/14 unit + 46/46 integration green.

#### Out of scope (separate tasks, post-merge)

- **3DS handling on the Accept response** — the `AcceptOutcome.payment` payload (with `requiresAction` + `clientSecret`) is currently unconsumed by the FE accept mutation. The BE sends it correctly; the FE just fires-and-forgets. Same shape as `useTicketCheckout`'s 3DS — should be wired separately. File as a follow-up task post-merge.
- **Pure-HATEOAS workflow rewrite** (Checkout/Confirm/Continue uniform interface) — discussed and rejected. Current role-interface design stays.
- **Add SignalR notification + `useCheckoutFlow` for apply-checkout** — explicitly **rejected** as ceremony-for-its-own-sake (SetupIntent confirmation is synchronous; no webhook gate to wait on; current `CheckoutAwaiting` loading state during the apply mutation is the right level of feedback).

#### Resume notes

- Migrations: no schema changes expected (PrepaidApplication.PaymentMethodId already exists). If something in entities changes, re-scaffold per CLAUDE.md.
- Phase marker decision (BE4) is locked: `IApplyCheckout` / `IAcceptCheckout` extend `ICheckout` (NOT `ICheckoutable`). Single declaration per strategy (drops the bare `ICheckout`). Single cast in dispatcher. Don't re-debate.
- Dispatcher methods named `ApplyCheckoutAsync` / `AcceptCheckoutAsync` (NOT `CheckoutForApplyAsync` etc — verb-as-prefix form is cleaner).
- Checkout return types are non-nullable end-to-end (dispatcher → service → endpoint → FE) — every workflow implements `ICheckout` once VenueHire is added, and the phase marker is the routing guard.
- Apply-side HATEOAS kept (BE13/FE5/FE6 in scope). FE Apply button stays contract-type-blind — branches on `opportunity.actions.checkout` presence. Discussed 2026-05-02; briefly considered dropping HATEOAS in favour of an FE `contract.type === "VenueHire"` switch, rejected — re-introducing contract-type coupling for *actions* is exactly what HATEOAS prevents (display rendering may still switch on type, but action choice should not).
- BE11 endpoint URL is locked: `POST /api/Application/opportunity/{oppId}/checkout` on `ApplicationController` — controller owns entity-being-written principle (the Application's PM is the eventual write). Don't re-debate by trying to put it on `OpportunityController`.
- The existing `IApplicationService.CheckoutAsync` callers will all need to switch to `AcceptCheckoutAsync` form (BE10); easy to miss because compile error is at the call sites, not the interface.
