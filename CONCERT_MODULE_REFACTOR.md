...
- **Delete `IManagerRepository.GetByConcertIdAsync` + `GetByApplicationIdAsync`** in
  ...
  and the `VenueManagerRepository` equivalent (confirm at impl time). Callers — the four
  workflow strategies, `OpportunityService`, `UpfrontConcertService`/`DeferredConcertService`,
  `OpportunityApplicationValidator` — move into `Concert.Application`/`Concert.Infrastructure`
  and resolve the Concert→UserId hop locally via `IConcertRepository` / `IOpportunityRepository`
  (add `GetArtistUserIdForConcertAsync`/`GetManagerUserIdsForApplicationAsync` as internal
  methods, Concert module-scoped only). Then call `IManagerModule.GetByIdAsync(userId)`. This
  is the "preferred fix option 1" from `CONCERT_MODULE_REFACTOR_DRAFT.md` — execute it as
  callers migrate, not as a big-bang rewrite. Track in a checkbox list during Stage 1 step 7.
- **Hot read paths with the two-query split that are measurably slower:** promote to a query
  handler inside `Concert.Application/Queries/` that uses `IReadDbContext` to do the full
  join and return a DTO. Don't add these speculatively — profile first. The draft's §
  "When the single-query version is worth keeping" is the north star.
...
This is the biggest extraction so far — 17 steps with a few deliberate checkpoints.
...
3. **Audit + delete cross-module methods on Identity repos.** Delete
   ...
   `ArtistManagerRepository` + `VenueManagerRepository`. Callers will temporarily not
   compile — that's fine, Step 7 fixes them. This surfaces the full caller list via the
   compiler rather than grep.
...
7. **Move Infrastructure layer** → `Concert.Infrastructure/`:
   ...
   - Rewrite callers of the deleted Identity repo methods (from Step 3) to use local
     Concert→UserId hops + `IManagerModule.GetByIdAsync`.
