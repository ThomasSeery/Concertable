...
public class ConcertApplicationVenueHireApiTests : IAsyncLifetime
{
    private readonly ApiFixture fixture;

    public ConcertApplicationVenueHireApiTests(ApiFixture fixture)
    {
    ...
    public async Task Accept_ShouldReturn400_WhenAlreadyAccepted()
    {
        ...
        // Act
        await client.PostAsync($"/api/ConcertApplication/accept/{TestConstants.VenueHire.ApplicationId}", (object?)null);
        await fixture.StripeClient.SendWebhookAsync();
        var response = await client.PostAsync($"/api/ConcertApplication/accept/{TestConstants.VenueHire.ApplicationId}", (object?)null);
    ...
    public async Task Accept_ShouldSettleAndCreateDraftConcertAndNotifyArtist()
    {
        ...
        // Assert
        var application = await client.GetAsync<ConcertApplicationDto>($"/api/ConcertApplication/{TestConstants.VenueHire.ApplicationId}");
    ...
    public async Task Accept_ShouldNotSettle_WhenPaymentFails()
    {
        ...
        var application = await fixture.CreateClient(TestConstants.VenueManager)
            .GetAsync<ConcertApplicationDto>($"/api/ConcertApplication/{TestConstants.VenueHire.ApplicationId}");
        Assert.Equal(ApplicationStatus.AwaitingPayment, application!.Status);
        Assert.Empty(fixture.NotificationService.DraftCreated);
    }
}
