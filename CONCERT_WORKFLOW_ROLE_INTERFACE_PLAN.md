...
| 22 | **Customer-side fallback parallel cleanup (carry-over)** | `CustomerPaymentModule.PayAsync` (ticket-purchase path for customers) still has the same `paymentMethodId ?? TryGetSavedPaymentMethodAsync(...) ?? throw` fallback. `IStripeAccountService.TryGetPaymentMethodAsync` only used by that one site. Same anti-pattern; same fix (frontend `ConcertCheckoutPage.tsx` always sends explicit PM, drop fallback, drop `TryGetPaymentMethodAsync`). Out of scope for #23 (concert workflow), but closely related — pick up next. |
